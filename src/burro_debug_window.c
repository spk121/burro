                 ...
                 gpointer       user_data)
{
	...
	BurroDebugWindow *window;
	gboolean running;

	window = BURRO_DEBUG_WINDOW (user_data);
...
{
    gboolean active = burro_app_window_get_active_flag();
    burro_app_window_set_active_flag(!active);
    if (active)
        gtk_widget_set_sensitive (GTK_WIDGET(debug_window_cur->debug_next_button), FALSE);
    else
    {
        gtk_widget_set_sensitive (GTK_WIDGET(debug_window_cur->debug_next_button), TRUE);
        gtk_label_set_text (debug_window_cur->update_rate_label, "paused");
    }
}
...
burro_debug_window_class_init (BurroDebugWindowClass *class)
{
    ...
    const char window_ui[] = "/com/lonelycactus/burroengine/debug-window.ui";
    
    gclass->dispose = burro_debug_window_dispose;

    gtkclass->delete_event = burro_debug_window_delete;
#if 0    
    gtkclass->window_state_event = burro_debug_window_state;
#endif

    gtk_widget_class_set_template_from_resource (gtkclass, window_ui);

    gtk_widget_class_bind_template_child (gtkclass, BurroDebugWindow, message_view);
    gtk_widget_class_bind_template_child (gtkclass, BurroDebugWindow, message_scrolled_window);
    gtk_widget_class_bind_template_child (gtkclass, BurroDebugWindow, repl_combobox);
    gtk_widget_class_bind_template_child (gtkclass, BurroDebugWindow, console_entry);
    gtk_widget_class_bind_template_child (gtkclass, BurroDebugWindow, vram_tree_view);
    gtk_widget_class_bind_template_child (gtkclass, BurroDebugWindow, update_rate_label);
    gtk_widget_class_bind_template_child (gtkclass, BurroDebugWindow, duty_cycle_label);
    gtk_widget_class_bind_template_child (gtkclass, BurroDebugWindow, debug_pause_button);
    gtk_widget_class_bind_template_child (gtkclass, BurroDebugWindow, debug_next_button);
    gtk_widget_class_bind_template_child (gtkclass, BurroDebugWindow, debug_peek_tree_view);
}
